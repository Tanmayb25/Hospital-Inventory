    ...
    Promise.all([labEquipmentExist])
       .then(()=>{
                ...
                labEquipment.save()
                    ...
                    .catch(error => res.status(500).send({msg:`there is some error: ${error}`}))
        }).catch(error => {
            return res.status(500).send({ msg:`there is some error: ${error}`})
        })
        }catch(err){
        res.status(500).json({msg:`there is some error: ${error}`});
    }
}
